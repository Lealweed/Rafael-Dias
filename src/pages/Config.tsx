    ...
    <div className="flex flex-col h-full w-full max-w-4xl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] uppercase tracking-widest text-gray-400 font-semibold mt-1">Verifique o status das integrações e variáveis de ambiente.</p>
        </div>
        <button 
          onClick={fetchHealth}
          className="px-4 py-2 bg-white text-sm font-bold text-[#2563EB] border border-gray-200 rounded-lg hover:bg-gray-50 shadow-sm"
        >
          Atualizar Status
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-200">
          <h2 className="text-lg font-bold text-[#111827]">Health Check de Sistema</h2>
        </div>
          ...
          {loading ? (
            ...
            <div className="space-y-6">
               <div className="flex items-center justify-between py-3 border-b border-gray-100">
                 <div>
                   <p className="font-bold text-gray-900">API Backend / Express</p>
                 </div>
                 <div className="flex items-center gap-2">
                   {healthStatus?.status === "ok" ? <CheckCircle2 className="w-5 h-5 text-green-500"/> : <XCircle className="w-5 h-5 text-red-500"/>}
                   <span className="text-sm font-medium text-gray-500">{healthStatus?.status === "ok" ? "Online" : "Offline"}</span>
                 </div>
               </div>
               ...
               <div className="flex items-center justify-between py-3 border-b border-gray-100">
                 <div>
                   ...
                   <p className="text-xs text-gray-500 mt-0.5">Verifica se as variáveis do Supabase estão configuradas</p>
                 </div>
                 <div className="flex items-center gap-2">
                   {healthStatus?.supabase === "configured" ? <CheckCircle2 className="w-5 h-5 text-green-500"/> : <XCircle className="w-5 h-5 text-red-500"/>}
                   <span className="text-sm font-medium text-gray-500">{healthStatus?.supabase || "Missing"}</span>
                 </div>
               </div>

               <div className="flex items-center justify-between py-3 border-b border-gray-100">
                 <div>
                   <p className="font-bold text-gray-900">n8n Webhook Configuration</p>
                   <p className="text-xs text-gray-500 mt-0.5">Credenciais para disparo ao n8n</p>
                 </div>
                 <div className="flex items-center gap-2">
               ...
               <div className="mt-8 bg-blue-50/50 rounded-xl p-5 border border-blue-100">
                 ...
                 <ol className="mt-4 list-decimal list-inside text-sm text-blue-800 space-y-2">
                   <li>Crie um projeto no Supabase</li>
                   <li>Rode o SQL de migração localizado em <code className="bg-white px-1.5 py-0.5 rounded text-blue-600 font-mono text-xs">supabase/migrations/00001_initial_schema.sql</code></li>
                   <li>Preencha as variáveis locais no arquivo de ambiente (baseado no <code className="bg-white px-1.5 py-0.5 rounded text-blue-600 font-mono text-xs">.env.example</code>)</li>
